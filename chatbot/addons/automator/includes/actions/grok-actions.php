 ...
 * Class Grok_Actions
 */
class Grok_Actions extends Action {
	...
	public function __construct() {
		$this->id    = 'grok';
		$this->group = __( 'Grok AI', 'wpbot-automator' );
	}
	...
	private function test_connection( $config, $trigger_data ) {
		...
		if ( $response_code >= 200 && $response_code < 300 ) {
			return array(
				'success' => true,
				'message' => 'Connection successful! Your xAI API key is valid.',
			);
		} else {
	...
	private function chat_completion( $config, $trigger_data ) {
		...
		$api_key     = isset( $config_values['api_key'] ) ? $this->parse_tokens( $config_values['api_key'], $trigger_data ) : '';
		$model       = isset( $config_values['model'] ) ? $config_values['model'] : 'grok-beta';
